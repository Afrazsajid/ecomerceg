...
import { useCart } from '@/app/context/cartcontext'; // Import the useCart hook
import { FaTrash } from "react-icons/fa";
import { client } from '@/sanity/lib/client'; // Assuming this is where your client is set up
import { PRODUCTS_Query } from '@/sanity/lib/quries'; // Assuming PRODUCTS_Query is already defined

const ShoppingCart: React.FC = () => {
  const { cart, loading, removeFromCart, updateQuantity, getTotalPrice, promoCode, setPromoCode, discount, applyPromoCode } = useCart();
  
  const [updatingItemId, setUpdatingItemId] = useState<string | null>(null); // Track which item is being updated
  const [products, setProducts] = useState<any[]>([]); // Store fetched product data
  ...
  const handleDeleteItem = (itemId: string, namee: string) => {
    console.log('Deleting item with id:', itemId);
    console.log('Deleting item with name:', namee);
    removeFromCart(itemId); // Make sure removeFromCart is using the correct itemId
  };
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const params = { slugs: cart.map(item => item.id) }; // Assuming item.id is the slug
        const fetchedProducts = await client.fetch(PRODUCTS_Query, params);
        console.log('Fetched Products:', fetchedProducts); // Check what is returned from the API
  ...
  const getpod = async()=>{
   return await client.fetch(PRODUCTS_Query,{search:"toy-train-set"})
    ...
    <div className="p-6 bg-gray-50">
      ...
      {cart.length === 0 ? (
        ...
        <div className="flex flex-col lg:flex-row space-x-6">
          <div className="flex-1 bg-white p-6 rounded-lg shadow-lg">
            {cart.map((item) => {
              const product = products.find(p => p.slug.current === item.id); // Find product by slug (id)

              if (!product) return null; // If product is not found, skip
